...
{
	//Swing Components
	JSpinner maxGenSp, numPlayersSp,iterationsSp, mutateSp, crossoverSp,  temptationSp, rewardSp, suckerSp, punishSp, TrustSp;
	JLabel maxGenLbl, numPlayersLbl,mutateLbl, crossoverLbl, iterationsLbl, temptationLbl, rewardLbl, suckerLbl, punishLbl, TrustLbl;
	JButton defaultBtn, saveBtn, restoreBtn;
	JDialog rulesDlg;
	
	//Rule settings
	private int gen, plyrs, itr, t, r, s, p;
	private double m, c, Trust;
	...
	public RulesDialog(final MenuFrame frame)
	{
		...
		if(exist)
		{
			...
			if(props.getProperty("c") != null)
				...
				c = 0;
			if(props.getProperty("Trust") != null)
				Trust = Double.parseDouble(props.getProperty("Trust"));
			else
				Trust = 1.0;
		}
		else
		...
		model = new SpinnerNumberModel(c, 0, 1, .01); 
		TrustSp = new JSpinner(model);
		TrustLbl = new JLabel("Trust:", SwingConstants.LEFT);
		...
		defaultBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) 
            {
                ...
                TrustSp.setValue(new Double(0.0));
            }
        });
		...
		restoreBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) 
            {
                ...
                TrustSp.setValue(new Double(Trust));
                rulesDlg.setVisible(false);
            }
        });
	...
	{ 
		...
		return new Rules(itr, t, s, r, p, m, c, Trust);
	}
	...
	private void save()
	{
        ...
        Trust = ((Double)TrustSp.getValue()).doubleValue();
       // Rules(int it, int t,int s, int r, int p, double m, double c, int gen, int pl)
        mf.setRules(new Rules(itr, t,s,r,p, m,c, gen, plyrs, Trust));
		...
		props.setProperty("Trust",Double.toString(Trust));
		try
		{
