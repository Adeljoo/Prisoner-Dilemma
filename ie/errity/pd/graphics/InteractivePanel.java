...
public class InteractivePanel extends JPanel implements ActionListener
{
	...
	//int Ris;
	//int Risk;
	JLabel Riskoftaking,Gainofgame,opponentScore, yourScore, opponentConfidence, yourConfidence ,opponentMove, yourMove, Opptxt, Opptxt2;
	...
	private int P1Total;
	private int P2Total;
	private double ConfidenceP1;
	private double ConfidenceP2;
	private int Risk,Gain;
	...
	private void scoresP()
	{
		...
		opponentScore = new JLabel("<html><font color=#FF0000 size=+1>Player 1's Total Score: " +  (new Integer(0)).toString() + "</font>");
		yourScore = new JLabel("<html><font color=#0000FF size=+1>Player 2's Total Score: " +  (new Integer(0)).toString() + "</font>");
		
		opponentConfidence = new JLabel("<html><font color=#FF0000 size=+1>Player 1's Total Confidence: " +  (new Double(0.0)).toString() + "</font>");
		yourConfidence = new JLabel("<html><font color=#0000FF size=+1>Player 2's Total Confidence: " +  (new Double(0.0)).toString() + "</font>");
		
		
		Riskoftaking = new JLabel("<html><font color=#FF0000 size=+1>Total Risk: " +  (new Integer(0)).toString() + "</font>");
	
		
		Gainofgame = new JLabel("<html><font color=#FF0000 size=+1> Total Gain: " +  (new Integer(0)).toString() + "</font>");
		...
		scoresPanel.add(opponentConfidence);
		scoresPanel.add(yourConfidence);
		
		scoresPanel.add(Gainofgame);
		scoresPanel.add(Riskoftaking);
    ...
    {	
    	...
    	if ("Play".equals(e.getActionCommand())) 
    	{
	    	...
	    	if(!invalid) 
	    	{
				...
				history = new BitSet();
				P2History = new BitSet();
				opponentScore.setText("<html><font color=#FF0000 size=+1>Player 1's Total Score: " +  (new Integer(P2Total)).toString() + "</font>");
				yourScore.setText("<html><font color=#0000FF size=+1>Player 2's Total Score: " +  (new Integer(P1Total)).toString() + "</font>");
    	...
    	else if ("C".equals(e.getActionCommand()) && (iteration < itMax))
    	{
			...
			Riskoftaking.setText("<html><font color=#FF0000 size=+1>Total Risk: " +  (new Integer(Risk)).toString() + "</font>");
			Gainofgame.setText("<html><font color=#0000FF size=+1>Total Gain: " +  (new Integer(Gain)).toString() + "</font>");
			
			opponentConfidence.setText("<html><font color=#FF0000 size=+1>Player 1's Confidence: " +  (new Double (ConfidenceP2)).toString() + "</font>");
			yourConfidence.setText("<html><font color=#0000FF size=+1>Player 2's Confidence: " +  (new Double (ConfidenceP1)).toString() + "</font>");
			if(move)
	...
	private boolean move(boolean P1move)
	{
		...
		Risk = rules.getP()-rules.getS();
		Gain = rules.getR()-rules.getP();
		if(P1move && P2move) //CC
		{
			P1Score = rules.getR();
			P2Score = rules.getR(); 
			ConfidenceP1+= rules.getConfidence()+0.1*0.8;	
			ConfidenceP2 += rules.getConfidence()+0.01*0.8;
		...
		else if(P1move && !P2move) //CD
		{
			P1Score = rules.getS();
			P2Score = rules.getT();
			ConfidenceP1+= rules.getConfidence()-0.01*0.8;
			ConfidenceP2 += rules.getConfidence()-0.1*0.8;
		}
		else if(!P1move && P2move) //DC
		{
			P1Score = rules.getT();
			P2Score = rules.getS();
			ConfidenceP1+= rules.getConfidence()+0.05*0.8;
			ConfidenceP2 += rules.getConfidence()+0.01*0.8;
			...
			final SwingWorker worker = new SwingWorker() 
			{
				public Object construct()
		        {
					...
					{	
						...
						P1Total += P1Score;							
						P2Total += P2Score;	
						opponentScore.setText("<html><font color=#FF0000 size=+1>Player 1's Total Score: " +  (new Integer(P1Total)).toString() + "</font>");
						yourScore.setText("<html><font color=#0000FF size=+1>Player 2's Total Score: " +  (new Integer(P2Total)).toString() + "</font>");						
					}
